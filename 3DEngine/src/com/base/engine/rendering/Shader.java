...
public class Shader{
	...
	public void updateUniforms(Transform transform, Material material, RenderingEngine renderingEngine){
		...
		for(int i = 0; i < resource.getUniformNames().size(); i++){
			...
			if(uniformName.startsWith("P_")){
				if(uniformName.equals("P_MVP"))
					setUniform(uniformName, MVPMatrix);
				else if(uniformName.equals("P_model"))
					setUniform(uniformName, worldMatrix);
				else if(uniformName.equals("P_ambient"))
                    setUniform(uniformName, renderingEngine.getAmbientLight());
                else if(uniformName.equals("P_directionalLight"))
                    setUniformDirectionalLight(uniformName, (DirectionalLight)renderingEngine.getActiveLight());
                else if(uniformName.equals("P_pointLight"))
                    setUniformPointLight(uniformName, (PointLight)renderingEngine.getActiveLight());
                else if(uniformName.equals("P_spotLight"))
                    setUniformSpotLight(uniformName, (SpotLight)renderingEngine.getActiveLight());
                else if(uniformName.equals("P_eyePos"))
                    setUniform(uniformName, renderingEngine.getMainCamera().getTransform().getTransformedPos());
                else
					throw new IllegalArgumentException(uniformName + " is not a valid predefined variable");
			}else{
				if(uniformType.equals("vec3")) {
                    ...
                    int samplerSlot = Texture.getTextureSlot(uniformName);
